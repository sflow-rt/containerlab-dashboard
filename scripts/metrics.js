...
// version: 1.0
// date: 8/3/2023
// description: Containerlab Dashboard
...
setFlow('cl_bytes',{value:'bytes',t:2});
setFlow('cl_flow',{
  keys:[
   'or:ipsource:ip6source',
   'or:ipdestination:ip6destination',
   'or:ipprotocol:ip6nexthdr',
   'or:tcpsourceport:udpsourceport:icmptype:icmp6type',
   'or:tcpdestinationport:udpdestinationport:icmpcode:icmp6code'
  ],
  value:'bytes',t:2}
);
...
setIntervalHandler(function(now) {
  ...
  var bps = flowCount('cl_bytes') * 8;  
  points['top-5-flows'] = calculateTopN('cl_flow',5,1,points.bps);
  points['top-5-interfaces'] = calculateTopInterfaces('cl_bytes',5); 
...
function getTopologyMetrics() {
  ...
  if(top && top.links) {
    for(name in top.links) {
      vals = topologyLinkMetric(name,'cl_bytes');
