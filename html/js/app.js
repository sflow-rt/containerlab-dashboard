$(function() { 
  ...
  function setNav(target) {
    ...
    window.sessionStorage.setItem('containerlab_dashboard_nav',target);
    window.history.replaceState(null,'',target);
  }

  var hash = window.location.hash;
  if(hash && $('.navbar .nav-item a[href="'+hash+'"]').length == 1) setNav(hash);
  else setNav(window.sessionStorage.getItem('containerlab_dashboard_nav') || $('.navbar .nav-item a').first().attr('href'));
  ...
  var db = {};
  var protos = {'6':'TCP','17':'UDP','1':'ICMP','58':'ICMP6'};
  $('#diameter').chart({
    type: 'trend',
    metrics: ['diameter'],
    stack:false,
  ...
  $('#topflows').chart({
    ...
    keyName: function(k,i) { return i == 2 ? protos[k] || k : k;},
  ...
  function updateTopology(top) {
    ...
    if(top.links) {
      for(edge in top.links) {
        ...
        top.nodes[entry.node1] = true;
        if(!nodes.get(entry.node1)) nodes.add({id:entry.node1,label:entry.node1});
        top.nodes[entry.node2] = true;
        if(!nodes.get(entry.node2)) nodes.add({id:entry.node2,label:entry.node2});
